...
export class HistoriqueComponent implements OnInit {
  ...
  ngOnInit(): void {
    const date = new Date();
    const date_debut = date.getFullYear() + '-' + (date.getMonth() + 1).toString().padStart(2, '0') + '-' + date.getDate().toString().padStart(2, '0') + 'T00:00:00';
    const date_fin = date.getFullYear() + '-' + (date.getMonth() + 1).toString().padStart(2, '0') + '-' + date.getDate().toString().padStart(2, '0') + 'T23:59:59';
  ...
  private async initMap(lat: number, lon: number, zoom: number) {
    ...
    if (this.positions.length > 0) {
      ...
      await this.positions.forEach((position: any) => this.createMarker(position));

      let positions: any = [];
      this.positions.forEach((position: any) => {
        positions.push([position.latitude, position.longitude]);
      });
      L.polyline(positions).addTo(this.map);
